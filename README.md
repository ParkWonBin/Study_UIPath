...
#### 엑셀 VBA, VB.NET 코드 TIP
```md
1. 메크로 녹화 방법
  1.1. 개발도구 리본에 표시
    - [파일]>[옵션]>[리본 사용자 지정]>[리본 메뉴 사용자 지정 = 개발도구 체크]
  1.2. 메크로 기록 클릭
  - [개발도구]>[코드]>[매크로 기록]
  - 매크로 이름 및 바로가기 버튼 지정
  1.3. 수행할 Action 진행
...
2. 매크로 조회 방법
  2.1. [Alt+F8] 메크로 창 보기
  2.2. 조회하고자 하는 매크로 클릭 > 편집
  2.3. 적당히 예외처리 및 리팩토리 진행
  2.4. 해당 매크로 실행 - 컴파일 잘되는지 확인
  2.5. vba -> vb.net 변환하여 Uipath Invoke Code에 넣기
...
- (1) Invoke Code 내에서 새로운 SUB 생성 불가.
