...
### 커스텀 함수
#### 목적 : Log함수 안에서 해당 함수를 쓰면, log와 값 저장을 동시에 할 수 있다.
- envSet("변수명","값") : 환경변수에 해당 값을 할당하고 "값"을 반환합니다.
- envApp("변수명","값") : 환경변수에 해당 값을 누적(Append)하고 "값"을 반환합니다.
- envGet("변수명") : 환경변수에서 구분자를 넣어 해당 "값"을 누적하고, "값"을 반환합니다.
- envPop("변수명") : 환경변수에서 구분자 기준 마지막 Item을 삭제하고, 해당 삭제된 값을 반환합니다.

```vb
' out_envSet
out_envSet = Function(str_varName As String, str_Value As String)
    Environment.SetEnvironmentVariable(str_varName,str_Value)
    Return str_Value
End Function

' "envSet('log','Test중') : " + envSet("log","Function Test중")
' "envGet('log') : "+ envGet("log")

```
```vb
' out_envGet
out_envGet = Function(str_varName As String)
    If String.IsNullOrWhiteSpace(environment.GetEnvironmentVariable(str_varName)) Then
            Throw New applicationException(String.Format("Environment에 {0}변수가 없습니다.",str_varName))
        Else
...
' "envApp('logTree','root') : " + envApp("logTree","root")
' "envApp('logTree','node1') : " + envApp("logTree","node1")
' "envApp('logTree','node2') : " + envApp("logTree","node2")
```
